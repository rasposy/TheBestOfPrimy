...
post_response(Req0, _State0) ->
    io:fwrite("[event_route (pid=~p)] post_response.~n", [self()]), 
    {ok, EncodedData, _} = cowboy_req:read_body(Req0),
    io:fwrite("[event_route (pid=~p)] encoded data ~p.~n", [self(), EncodedData]), 
    DecodedData = jiffy:decode(EncodedData),
    io:fwrite("[event_route (pid=~p)] DecodedData data ~p.~n", [self(), DecodedData]), 
    % we need to decode to get only the content of the json data
    Decoded_array = decoded_data(DecodedData),
    io:fwrite("[event_route (pid=~p)] Decoded_array  ~p.~n", [self(), Decoded_array]), 
    ...
    case Decoded_array of
        [] ->
            {R, Code} = server:post(Decoded_array),
            Reply = {response, R}
    end,

    io:fwrite("[event_route (pid=~p)] answer from server:post is ~p.~n", [self(), R]), 
